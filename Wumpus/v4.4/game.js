...
function initializeGeneticAlgorithm() {
    ...
    // Ajustar o comprimento do cromossomo baseado no tamanho do tabuleiro
...
function evaluateFitness(chromosome) {
    ...
    let fitness = 0;
    let hasGold = false;
    let goldPosition = findGoldPosition();
    
    for (let i = 0; i < chromosome.length; i++) {
        const action = chromosome[i];
        const result = simulator.move(action);
        
        if (result === 'death') {
            fitness -= 1000; // Penalidade por morte
            break;
        }
        ...
        if (result === 'gold') {
            ...
            hasGold = true;
            goldPosition = null; // Já pegou o ouro
        }
        
        // Recompensa por estar perto do ouro (se ainda não pegou)
        if (!hasGold && goldPosition) {
            const distanceToGold = Math.abs(simulator.agent.x - goldPosition.x) + 
                                  Math.abs(simulator.agent.y - goldPosition.y);
            fitness += (wumpusWorld.size * 2 - distanceToGold) * 5;
        }
        
        // Recompensa por retornar à base com o ouro
        if (hasGold && simulator.isAtStart()) {
            fitness += 1000; // Recompensa por retornar com o ouro
            break;
        }
        
        // Recompensa por estar perto da base (se tem o ouro)
        if (hasGold) {
            const distanceToBase = Math.abs(simulator.agent.x) + Math.abs(simulator.agent.y);
            fitness += (wumpusWorld.size * 2 - distanceToBase) * 5;
        }
    }
...
function executeBestSequenceLogic() {
    ...
    // Verifica se o objetivo foi alcançado após o movimento
...
function agentV3Logic() {
    // Fase de Evolução
    if (evolutionPhase) {
        // Inicializa a população na primeira geração
        if (currentGeneration === 0) {
            initializeGeneticAlgorithm();
        }

        // Se ainda estamos na fase de evolução
        if (currentGeneration < MAX_GENERATIONS) {
            // Processa uma geração por chamada (mantém a estrutura original)
            evaluatePopulation();
            ...
            // A condição de parada agora é mais clara
            if (bestFitness >= 1998) {
            ...
            // Se o limite de gerações foi atingido, a evolução termina
            gameMessage.textContent = "Agente V3: Evolução concluída. Executando melhor sequência.";
            evolutionPhase = false;
        }

        // Se a evolução terminou (por solução ótima ou limite de gerações), prepare-se para a execução
        if (!evolutionPhase) {
            stopAutoPlay(); // Interrompe o loop de evolução
            // Inicia um novo intervalo para a fase de execução
            autoPlayInterval = setInterval(() => {
                executeBestSequenceLogic();
            }, velocity);
        }
    }
}
...
function startAutoPlay() {
    ...
    autoPlayInterval = setInterval(() => {
        ...
        if (agent === 'V1') {
            ...
            // O V3 agora tem sua própria lógica de controle de fase
            if (evolutionPhase) {
                agentV3Logic(); // Executa uma etapa da evolução
            } else {
                executeBestSequenceLogic(); // Passa para a execução
            }
        }
    }, velocity);
}
...
function autoPlayOn() {
    ...
    if (agent === 'V2') {
        ...
        // Resetar todas as variáveis específicas do V3
        population = [];
        currentGeneration = 0;
        bestIndividual = null;
    ...
    renderBoard();
    // A função startAutoPlay() agora será a única a ter o setInterval,
    // e ela decidirá qual lógica rodar.
    startAutoPlay();
}
