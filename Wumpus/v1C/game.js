...
resetGameBtn.addEventListener('click', () => {
    ...
    wumpusWorld.board[0][0].isVisited = true;
    renderBoard();
    gameMessage.textContent = 'Jogo resetado. Mova o agente.';
    hasGold = false;
    updateUIForMode();
});
...
modeToggleBtn.addEventListener('click', () => {
    gameMode = gameMode === 'manual' ? 'automatico' : 'manual';
    modeToggleBtn.textContent = `Modo: ${gameMode.charAt(0).toUpperCase() + gameMode.slice(1)}`;
...
// Checar o estado do jogo (vitória, derrota) - (código inalterado)
function checkGameState() {
    const { x, y } = wumpusWorld.agent;
    const cellContent = wumpusWorld.board[y][x];

    if (cellContent.objects.includes('pit') || cellContent.objects.includes('wumpus')) {
        gameMessage.textContent = 'Você caiu em um poço ou foi devorado pelo Wumpus. Fim de jogo!';
        gameMessage.style.color = 'red';
        revealBoard();
        setTimeout(() => {
            wumpusWorld.reset();
    ...
    if (cellContent.objects.includes('gold')) {
        gameMessage.textContent = 'Você encontrou o Ouro! Retorne para a casa inicial.';
        gameMessage.style.color = 'blue';
        hasGold = true;
    }

    if (hasGold && x === 0 && y === 0) {
        gameMessage.textContent = 'Parabéns! Você VENCEU!';
...
// Controles para o modo manual (código inalterado)
document.getElementById('moveNorth').addEventListener('click', () => moveAgent('north'));
document.getElementById('moveSouth').addEventListener('click', () => moveAgent('south'));
document.getElementById('moveWest').addEventListener('click', () => moveAgent('west'));
document.getElementById('moveEast').addEventListener('click', () => moveAgent('east'));
...
function updateUIForMode() {
    if (gameMode === 'manual') {
        ...
        // Futura lógica para modo automático
    }
}