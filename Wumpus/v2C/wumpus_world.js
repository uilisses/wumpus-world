...
class WumpusWorld {
    ...
    placeObjects() {
        ...
        // --- Lógica de cálculo de poços alterada ---
        const totalCells = this.size * this.size;
        let pitPercentage;

        if (this.size < 10) {
            pitPercentage = 0.10; // 10% para tabuleiros menores que 10x10
        } else {
            pitPercentage = 0.05; // 5% para tabuleiros maiores ou iguais a 10x10
        }

        const numPits = Math.ceil(totalCells * pitPercentage);
